...
export class RoomScreen implements Screen {
  ...
  private drawNextCollectionGoal(g: CanvasRenderingContext2D): void {
    const store = this.ctx.store;
    const next = store.nextCollectionMilestone();
    ...
    if (!next) {
      drawText(g, t('collection.crowned'), centerX, box.y + 20, 1.75, GOLD, {
        align: 'center',
        glow: GOLD,
        glowBlur: 4,
      });
      drawText(g, owned + ' / ' + store.totalCollectibles(), centerX, box.y + 46, 1.3, INK, { align: 'center' });
      bar(g, centerX - 116, box.y + box.h - 13, 232, 7, 1, GOLD);
      return;
    }

    drawText(g, t('collection.nextArcadeUpgrade'), centerX, box.y + 11, 1.65, MAGENTA, {
      align: 'center',
      glow: MAGENTA,
      glowBlur: 2,
    });
    ...
    drawText(g, value, centerX, box.y + 39, valueScale, GOLD, { align: 'center' });
    bar(g, centerX - 116, box.y + box.h - 13, 232, 7, owned / next.milestone.threshold, CYAN);
  }
